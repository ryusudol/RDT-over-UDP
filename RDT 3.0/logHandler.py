...
class logHandler:
    ...
    def startLogging(self, filename):
        ...
        self.loggingProc = Process(
            target=logFileWorker, args=(self.logQueue, self.endEvent, self.filename)
        )
    ...
    def writePkt(self, pktNum, event):
        ...
        strToWrite = "{:1.3f} pkt: {} | {}\n".format(
            time() - self.startTime, pktNum, event
        )
    ...
    def writeAck(self, ackNum, event):
        ...
        strToWrite = "{:1.3f} ACK: {} | {}\n".format(
            time() - self.startTime, ackNum, event
        )
        self.logQueue.put(strToWrite)
    ...
    def writeTimeout(self, ackNum):
        ...
        strToWrite = "{:1.3f} pkt: {} | {}\n".format(
            time() - self.startTime, ackNum, "TIMEOUT"
        )
        self.logQueue.put(strToWrite)
